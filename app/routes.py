...
def create_account():
    ...
    account_id = str(uuid.uuid4())[:8]  # Short UUID
...
# Deposit money
@routes.route('/deposit', methods=['POST'])
def deposit():
...
# Withdraw money
@routes.route('/withdraw', methods=['POST'])
def withdraw():
    account_id = request.json.get('account_id')
    amount = request.json.get('amount', 0)
    if account_id not in data["accounts"]:
...
@routes.route('/transfer', methods=['POST'])
def transfer():
...
# Get account details
@routes.route('/account/<account_id>', methods=['GET'])
